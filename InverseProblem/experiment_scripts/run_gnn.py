...
p.add(
    '-c', '--config', required=False, default='InverseProblem/config/init_state_gnn_p.ini', is_config_file=True, help='Path to config file.'
)

p.add_argument(
    '--logging_root', type=str, default="./log/" , 
...
p.add_argument(
    '--experiment_name', type=str, default="GNN",
    help='Name of subdirectory in logging_root where summaries and checkpoints will be saved.'
)

p.add_argument(
    '--path_to_data', type=str, 
    default='./data', help='root for logging'
)
...
# load solver arguments
p.add_argument(
    '--solver_path', type=str, 
    default='./data/model_zoo/gnn_solver.pth'
)

p.add_argument(
    '--edge_features', nargs='+', type=str, 
    default=['dist','direction']
)

# load inverse problem arguments
...
p.add_argument(
    '--outmost_nonlinearity', type=str, default='sigmoid',
...
p.add_argument('--store', action='store_true', default=True)
...
fname_vars = [
    'experiment_name', 'repeat_time', 'loss_type', 'progressive', 'sensor_num', 'time_steps', 'obversation_step', 'start_observation_index', 'gradient_clip', 'lr', "reg", "lr_decay", "lr_decay_steps"
]
opt.experiment_name = ''.join([f'{k}_{vars(opt)[k]}_|_'.replace('[', '(').replace(']',')') for k in fname_vars])[16:-1]
print('experiment_name:', opt.experiment_name)
...
if opt.noprior:
    ...
    if opt.prior_type != "both":
        # load prior model
        prior = modules.CoordinateNet_autodecoder(
            latent_size=64, nl='relu', in_features=64+2, out_features=1,
            hidden_features=256, num_hidden_layers=6, num_pe_fns=3,
            w0=60, use_pe=True, skip_connect=None, dataset_size=10000,
            outmost_nonlinearity=opt.outmost_nonlinearity,
            outermost_linear=not(opt.outmost_nl)
        ).to(device)
        # print('prior:', prior)
        ...
        print('prior_path:', opt.prior_path)
    
        # load prior
        checkpoint_prior = torch.load(
            opt.prior_path,
        ...
        # load density prior model
        prior_density = modules.CoordinateNet_autodecoder(
            latent_size=64, nl='relu', in_features=64+2, out_features=1,
            hidden_features=256, num_hidden_layers=6, num_pe_fns=3,
            w0=60,use_pe=True,skip_connect=None,dataset_size=10000,
            outmost_nonlinearity=opt.outmost_nonlinearity,
        ...
        # load init state prior model
        prior_init_state = modules.CoordinateNet_autodecoder(
            latent_size=64, nl='relu', in_features=64+2, out_features=1,
            hidden_features=256, num_hidden_layers=6, num_pe_fns=3,
            w0=60,use_pe=True,skip_connect=None,dataset_size=10000,
            outmost_nonlinearity=opt.outmost_nonlinearity,
...
# load gnn model
gnn_solver = gnn_module.mesh_PDE(
    edge_dim=3, node_dim=4, latent_dim = 256,
    num_steps=10, layer_norm=True, nl='relu', var=0, 
    batch_norm=False, normalize=True, encoder_nl='relu', diffMLP=True
).to(device)
...
print('gnn path:', opt.solver_path)
...
inverse_gnn.test_inverse(
    prior=prior, gnn_solver=gnn_solver, graph_update_fn=graph_update_fn, mask_type=opt.mask_type, start_index=opt.start_index,
    ...
    gradient_clip=opt.gradient_clip, lr_decay_type=opt.lr_decay_type, reg=opt.reg, device=device, repeat_time=opt.repeat_time
)
